  ...
  return (
    <div className="rounded-xl border border-[#E4E4E7] bg-white p-5 shadow-xs">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-sm font-semibold uppercase tracking-wider text-[#71717A]">Portal Distribution</h3>
        <button type="button" onClick={() => setPreviewOpen((v) => !v)} className="text-xs font-semibold text-[#09090B] hover:underline">
          {previewOpen ? "Hide payload preview" : "Preview payload"}
        </button>
      </div>
      {previewOpen && (
        <div className="mb-4 rounded-lg bg-[#FAFAFA] border border-[#E4E4E7] p-3 text-xs text-[#52525B]">
      ...
      <div className="space-y-3">
        {rows.map((row) => (
          <div key={row.provider} className="rounded-lg border border-[#E4E4E7] p-3 text-sm">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <span className="font-semibold text-[#09090B]">{row.provider.replaceAll("_", " ")}</span>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-[#71717A]">{row.connectionStatus ? row.connectionStatus.replaceAll("_", " ") : "NOT CONFIGURED"}</span>
              </div>
            </div>
            <div className="mt-1.5 text-xs text-[#71717A]">
            ...
            {row.listing?.externalListingId && (
              <div className="mt-1 text-xs text-[#52525B]">External ID: {row.listing.externalListingId}{row.listing.externalUrl && <> · <a href={row.listing.externalUrl} target="_blank" rel="noreferrer" className="text-[#09090B] font-medium hover:underline">View listing</a></>}</div>
            )}
            {row.listing?.lastSyncedAt && <div className="mt-1 text-xs text-[#71717A]">Last sync: {new Date(row.listing.lastSyncedAt).toLocaleString("en-IN")}</div>}
            ...
            {row.operations.length > 0 && (
              <div className="mt-2 space-y-1">
                {row.operations.map((op) => (
                  <div key={op.id} className="flex items-center justify-between rounded bg-[#FAFAFA] border border-[#E4E4E7] px-2 py-1 text-xs">
                    ...
                    {canEdit && op.status === "RETRYABLE" && (
                      <button type="button" disabled={busy === op.id} onClick={() => retryOperation(op.id)} className="rounded border border-[#E4E4E7] px-2 py-0.5 font-semibold text-[#09090B] hover:bg-zinc-50">{busy === op.id ? "Retrying…" : "Retry"}</button>
                    )}
                  </div>
                ))}
              </div>
            ...
            {canEdit && (
              <div className="mt-2 flex flex-wrap gap-2">
                {!row.listing ? (
                  <button type="button" disabled={busy === row.provider} onClick={() => addToProvider(row.provider)} className="rounded border border-[#E4E4E7] px-2.5 py-1 text-xs font-semibold text-[#09090B] hover:bg-zinc-50">{busy === row.provider ? "Adding…" : "Add to distribution"}</button>
                ) : (
                  <>
      ...
      <p className="mt-3 text-xs text-[#71717A] border-t border-[#E4E4E7] pt-2.5">Contract-only integration: publish/update/deactivate remain blocked until an authorized partner connection reports AVAILABLE. No listings are sent to any provider by this CRM.</p>
    </div>
  );
}

function ActionButton({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return <button type="button" disabled={active} onClick={onClick} className="rounded border border-[#E4E4E7] px-2.5 py-1 text-xs font-semibold text-[#09090B] hover:bg-zinc-50 disabled:opacity-60">{active ? "…" : label}</button>;
}
