...
export function PropertyGallery({ propertyId, propertyTitle, legacyCoverImage }: { propertyId: string; propertyTitle: string; legacyCoverImage: string | null }) {
  ...
  if (error) {
    return (
      <div className="relative h-72 w-full overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-50 sm:h-96 flex items-center justify-center text-sm text-zinc-500">
  ...
  if (images === null) {
    return (
      <div className="relative h-72 w-full overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-50 sm:h-96">
  ...
  return (
    <div className="space-y-3">
      <div className="relative h-72 w-full overflow-hidden rounded-xl border border-zinc-200 bg-zinc-50 sm:h-96">
        {heroSrc ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-sm text-zinc-500">No photo uploaded for this property</div>
        )}
      ...
      {orderedActive.length > 0 && (
        <div className="flex gap-2 overflow-x-auto pb-1" role="list" aria-label="Image thumbnails">
          {orderedActive.map((img, idx) => (
            <div key={img.id} role="listitem" className="group relative h-16 w-20 shrink-0 overflow-hidden rounded-xl border border-zinc-200 bg-zinc-50">
              ...
              {img.isCover && (
                <span className="absolute left-0.5 top-0.5 rounded bg-[#0A0A0A] px-1 text-[9px] font-bold text-white">★ Thumbnail</span>
              )}
              {canManage && (
      ...
      {canManage && orderedActive.length > 0 && (
        <ul className="space-y-1.5">
          {orderedActive.map((img) => (
            <li key={img.id} className="flex items-center gap-2 rounded-xl border border-zinc-200 bg-zinc-50 px-2.5 py-1.5 text-xs">
              {captionDraftId === img.id ? (
                <>
                  <input
                    ...
                    className="flex-1 rounded-lg border border-zinc-200 bg-white px-2 py-1 text-xs text-zinc-900"
                  />
                  <button className="text-zinc-900 font-semibold hover:underline" onClick={() => saveCaption(img.id)} disabled={busyId === img.id}>Save</button>
                  <button className="text-zinc-500" onClick={() => setCaptionDraftId(null)}>Cancel</button>
                </>
              ) : (
                <>
                  <span className="flex-1 truncate text-zinc-700">{img.caption || <span className="text-zinc-400">No caption</span>}</span>
                  <button
                  ...
                  <label className="flex cursor-pointer items-center gap-1 text-zinc-500 hover:text-zinc-900">
      ...
      {canUpload && (
        <div>
          ...
          {showUploader && (
            <div className="mt-3 rounded-xl border border-zinc-200 bg-white p-4 shadow-xs">
      ...
      {lightboxIndex !== null && orderedActive.length > 0 && (
        <Dialog open onClose={() => setLightboxIndex(null)} title={propertyTitle} wide>
          <div className="relative">
            ...
            {orderedActive[lightboxIndex]?.caption && <p className="mt-2 text-center text-sm text-zinc-500">{orderedActive[lightboxIndex].caption}</p>}
            <div className="mt-3 flex items-center justify-between">
              ...
              <span className="text-xs text-zinc-500">{lightboxIndex + 1} / {orderedActive.length}</span>
