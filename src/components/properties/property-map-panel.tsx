  ...
  return (
    <div className="rounded-xl border border-[#E4E4E7] bg-white p-5 shadow-xs">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="flex items-center gap-2 text-sm font-semibold uppercase tracking-wider text-[#71717A]">
          <MapPin className="h-4 w-4 text-[#09090B]" /> Location
        </h3>
        <Badge tone={GEOCODE_STATUS_TONE[location.geocodeStatus] ?? "slate"}>{location.geocodeStatus.replace(/_/g, " ")}</Badge>
      </div>

      <p className="text-sm text-[#52525B]">{location.formattedAddress ?? destinationAddress}</p>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        <a href={mapHref} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-xl bg-white border border-[#E4E4E7] px-3 py-1.5 text-xs font-semibold text-[#09090B] hover:bg-zinc-50">
          <ExternalLink className="h-3.5 w-3.5" /> Open in Google Maps
        </a>
        <button onClick={copyLocation} className="inline-flex items-center gap-1.5 rounded-xl border border-[#E4E4E7] bg-white px-3 py-1.5 text-xs font-semibold text-[#52525B] hover:bg-zinc-50">
          <Copy className="h-3.5 w-3.5" /> Copy Location
        </button>
      </div>

      {canManage && (
        <div className="mt-4 space-y-3 border-t border-[#E4E4E7] pt-3">
          ...
          {!capabilities?.configured && <p className="text-[11px] text-[#71717A]">Re-geocoding requires Maps to be configured (currently disabled).</p>}

          <div>
            <label className="mb-1 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wide text-[#71717A]">
