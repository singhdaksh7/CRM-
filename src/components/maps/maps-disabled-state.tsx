    ...
    <div
      className={
        compact
          ...
          : "flex flex-col items-center justify-center rounded-2xl border border-dashed border-amber-200 bg-amber-50/40 py-8 px-4 text-center"
      }
    >
      ...
      <div className={compact ? "" : "mt-2"}>
        <p className="text-sm font-semibold text-zinc-900">Maps integration is not configured</p>
        <p className="mt-0.5 max-w-sm text-xs text-zinc-500">Address search and map previews will be available once Google Maps is connected. You can still enter the address manually.</p>
      </div>
      {role === "ADMIN" && (
      ...
      {role === "DATA_MANAGER" && <p className="mt-2 text-xs text-zinc-500">Contact your Admin to enable maps.</p>}
    </div>
  );
}
