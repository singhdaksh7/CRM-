...
function QuickActionButton({ href, icon: Icon, label, tone = "default" }: { href: string; icon: typeof Phone; label: string; tone?: "default" | "whatsapp" }) {
  return (
    <a
      href={href}
...
export function QuickActions({
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {clientPhone && <QuickActionButton href={`tel:${clientPhone}`} icon={Phone} label="Call Client" />}
      ...
      {ownerPhone && <QuickActionButton href={`tel:${ownerPhone}`} icon={Phone} label="Call Owner" />}
