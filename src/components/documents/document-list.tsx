...
export function DocumentList({ documents, loading, onOpen }: { documents: DocumentRecord[] | null; loading: boolean; onOpen: (doc: DocumentRecord) => void }) {
  ...
  return (
    <>
      {/* Desktop table */}
      <div className="hidden overflow-x-auto rounded-xl border border-[#E4E4E7] bg-white shadow-xs sm:block">
        <table className="min-w-full divide-y divide-[#E4E4E7] text-sm">
          ...
          <tbody className="divide-y divide-[#F4F4F5] text-[#52525B]">
            {documents.map((doc) => (
              <tr key={doc.id} className="hover:bg-[#F4F4F5] transition-colors">
                <td className="px-4 py-3.5">
                  <button onClick={() => onOpen(doc)} className="flex items-center gap-2 text-left font-semibold text-[#09090B] hover:underline">
                    <FileText className="h-4 w-4 shrink-0 text-[#09090B]" />
                ...
                <td className="px-4 py-3.5 text-xs text-[#71717A]">{formatDate(doc.createdAt)}</td>
                ...
                <td className="px-4 py-3.5 text-right">
                  <button onClick={() => onOpen(doc)} className="text-xs font-semibold text-[#09090B] hover:underline">
                    Open
                  </button>
                </td>
              </tr>
      ...
      <ul className="space-y-3 sm:hidden">
        {documents.map((doc) => (
          <li key={doc.id}>
            <button onClick={() => onOpen(doc)} className="block w-full rounded-xl border border-[#E4E4E7] bg-white p-4 text-left shadow-xs">
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <FileText className="h-4 w-4 shrink-0 text-[#09090B]" />
                  <span className="truncate font-semibold text-[#09090B]">{doc.originalFilename ?? doc.fileName}</span>
                </div>
                <Badge tone={STATUS_TONE[doc.status]}>{doc.status}</Badge>
              </div>
              <div className="mt-2 flex flex-wrap items-center gap-1.5 text-xs text-[#71717A]">
                <Badge tone={SENSITIVE_CATEGORIES.has(doc.category) ? "red" : "slate"}>{CATEGORY_LABELS[doc.category]}</Badge>
              ...
              <p className="mt-1.5 text-xs text-[#71717A]">{ENTITY_LABELS[doc.entityType]} · Uploaded by {doc.uploadedBy?.name ?? "-"}</p>
            </button>
          </li>
        ))}
      </ul>
...
function LinkedEntityCell({ doc }: { doc: DocumentRecord }) {
  ...
  if (!route) return <span className="text-xs text-[#71717A]">{ENTITY_LABELS[doc.entityType]}</span>;
  return (
    <Link href={route} className="flex items-center gap-1 text-xs text-[#09090B] font-medium hover:underline" onClick={(e) => e.stopPropagation()}>
