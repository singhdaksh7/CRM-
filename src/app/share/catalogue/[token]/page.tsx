...
export default async function PublicCataloguePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA]">
      <header className="flex items-center justify-between border-b border-[#E4E4E7] bg-white px-4 py-3 sm:px-8 shadow-xs">
        <div className="flex items-center gap-2">
          {dto.brokerageLogoUrl ? (
            ...
            <Building className="h-5 w-5 text-[#09090B]" />
          )}
          <span className="text-sm font-bold text-[#09090B]">{dto.brokerageName}</span>
        </div>
        {dto.brokerageContactPhone && (
          <a href={`tel:${dto.brokerageContactPhone}`} className="flex items-center gap-1 text-xs font-semibold text-[#09090B] hover:underline">
