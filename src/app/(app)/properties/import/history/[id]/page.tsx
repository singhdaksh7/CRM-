...
export default async function ImportDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const job = await prisma.importJob.findFirst({ where: { id, organizationId: getOrganizationId(session.user), entityType: "PROPERTIES" }, include: { records: { orderBy: { rowNumber: "asc" }, take: 5000 }, createdBy: { select: { name: true } } } }); if (!job) notFound();
