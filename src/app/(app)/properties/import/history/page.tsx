...
export default async function ImportHistoryPage() {
  ...
  const jobs = await prisma.importJob.findMany({ where: { organizationId: getOrganizationId(session.user.id), entityType: "PROPERTIES" }, include: { createdBy: { select: { name: true } } }, orderBy: { createdAt: "desc" }, take: 100 });
