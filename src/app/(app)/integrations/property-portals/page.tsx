...
export default async function PropertyPortalsPage() {
  ...
  return <div className="space-y-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-2xl font-bold text-[#1B2430]">Property Portals</h1><p className="mt-1 text-sm text-[#596579]">Organization-scoped, contract-only connections. Credentials and raw provider responses are never shown here.</p></div><div className="flex gap-2"><Link href="/integrations/property-portals/conflicts" className="rounded border px-3 py-2 text-sm font-semibold text-[#3366FF]">Sync conflicts →</Link></div></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{PROPERTY_PORTAL_PROVIDERS.map((provider) => <ConnectionCard key={provider} provider={provider} capabilities={propertyPortalRegistry[provider]} initial={byProvider.get(provider)} canEdit={session!.user.role === "ADMIN"} webhookUrl={provider === "HOUSING" ? getHousingWebhookUrl() : undefined} lastEvent={provider === "HOUSING" && lastHousingEvent ? { receivedAt: lastHousingEvent.receivedAt, status: lastHousingEvent.ingestionStatus } : undefined} housingImportHref={provider === "HOUSING" && canImportHousingLeads ? "/integrations/property-portals/housing-import" : undefined} />)}</div></div>;
}
