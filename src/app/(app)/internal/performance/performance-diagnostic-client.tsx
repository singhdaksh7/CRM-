...
type Query = { model: string; operation: string; duration: number; calls: number; resultSize: string; scope: string; filterFields: string; indexed: string };
type Benchmark = { total: number; metrics: Record<string, Metric>; queries: Query[]; queryStats?: { total: number; maxConcurrent: number; byScope: Record<string, number> } };
...
export function PerformanceDiagnosticClient({ previewDeployment, variant = "experimental" }: { previewDeployment: string; variant?: "baseline" | "experimental" }) {
  ...
  const dashboardQueries = Object.values(dashboardSamples.flatMap((sample) => sample.queries).reduce<Record<string, Query & { samples: number }>>((all, query) => { const key = `${query.scope}|${query.model}|${query.operation}|${query.resultSize}|${query.filterFields}`; const current = all[key] ?? { ...query, duration: 0, calls: 0, samples: 0 }; current.duration += query.duration; current.calls += query.calls; current.samples += 1; all[key] = current; return all; }, {})).map((query) => ({ ...query, duration: query.duration / dashboardSamples.length, calls: query.calls / dashboardSamples.length }));
  const dashboardQueryStats = { total: mean(dashboardSamples.map((sample) => sample.queryStats?.total ?? 0)), maxConcurrent: Math.max(0, ...dashboardSamples.map((sample) => sample.queryStats?.maxConcurrent ?? 0)) };
  ...
  const copy = async () => {
    const report = ["# KP CRM DEEP BENCHMARK", "", `Variant: ${variant}`, `Preview: ${previewDeployment}`, "", "## Architecture Boundaries", "Proxy: separate execution boundary; cannot share RSC request state.", "Auth.js: handler authentication includes authoritative JWT/user validation.", "RSC: React cache() deduplicates app-layout/page auth only within one RSC tree.", "", "## Server Benchmark", "Metric | Mean wall-clock", ...TARGETS.map((name) => `${name} | ${fmt(current(name))}`), "", "## Database Baseline", "Metric | Mean | Median", ...databaseMetrics.map(([name, average, middle]) => `${name} | ${fmt(average)} | ${fmt(middle)}`), "", "## Dashboard Query Count", `Total DB operations | ${dashboardQueryStats.total.toFixed(1)}`, `Maximum concurrent queries | ${dashboardQueryStats.maxConcurrent}`, "", "## Dashboard Waterfall", "Operation | Duration | Calls | Sequential/Parallel", ...Object.entries(dashboardMetrics).map(([name, item]) => `${name} | ${fmt(item.duration)} | ${item.calls} | ${item.parallel ? "Parallel" : "Sequential"}`), "", "## Dashboard Prisma Query Waterfall", "Scope | Operation | Duration | Calls | Result size | Filter fields | Indexed", ...dashboardQueries.map((query) => `${query.scope} | ${query.model}.${query.operation} | ${fmt(query.duration)} | ${query.calls.toFixed(1)} | ${query.resultSize} | ${query.filterFields} | ${query.indexed}`), "", "## Real Browser Route Latency", "Route | Mean wall-clock", ...ROUTES.map((path) => `${path} | ${fmt(mean(routeResults[path] ?? []))}`), "", "Production changed: NO"].join("\n");
  ...
  return <section className="mx-auto max-w-6xl space-y-6">
    ...
    {!!results.length && <><Table title="Server benchmark — this deployment" headers={["Metric", "Mean wall-clock"]} rows={TARGETS.map((name) => [name, fmt(current(name))])} /><Table title="Database baseline — 10 warm samples" headers={["Operation", "Mean wall-clock", "Median wall-clock"]} rows={databaseMetrics.map(([name, average, middle]) => [name, fmt(average), fmt(middle)])} /><Table title="Dashboard query count — ten-sample summary" headers={["Metric", "Value"]} rows={[["Total DB operations (mean)", dashboardQueryStats.total.toFixed(1)], ["Maximum concurrent queries", String(dashboardQueryStats.maxConcurrent)]]} /><Table title="Dashboard branch waterfall — mean per sample" headers={["Operation", "Duration", "Calls", "Scheduling"]} rows={Object.entries(dashboardMetrics).map(([name, item]) => [name, fmt(item.duration), String(item.calls), item.parallel ? "Parallel" : "Sequential"])} /><Table title="Dashboard Prisma query waterfall — mean per sample" headers={["Scope", "Model.operation", "Duration", "Calls", "Result size", "Filter fields", "Indexed?"]} rows={dashboardQueries.map((query) => [query.scope, `${query.model}.${query.operation}`, fmt(query.duration), query.calls.toFixed(1), query.resultSize, query.filterFields, query.indexed])} /><Table title="Real browser route latency — this deployment" headers={["Route", "Mean wall-clock"]} rows={ROUTES.map((path) => [path, fmt(mean(routeResults[path] ?? []))])} /></>}
  </section>;
}
