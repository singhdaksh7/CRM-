...
export function PerformanceDiagnosticClient({ previewDeployment, variant = "experimental" }: { previewDeployment: string; variant?: "baseline" | "experimental" }) {
  ...
  const databaseMetrics = ["databaseSelect1", "databaseUserLookup1", "databaseUserLookup2", "databaseOrganizationLookup", "databaseLeadCount"].map((name) => [name, mean(databaseSamples.map((sample) => sample.metrics[name]?.duration ?? 0))] as const);
