...
- **Public catalogue location privacy**: client-facing catalogue responses never include a street address, flat/unit number, entry instructions, or any GPS coordinate (exact or reduced-precision). Locality text (`area`) is the only location shown. Staff flags and `Property.publicLocationMode` cannot override that. Exact coordinates stay on the authenticated executive catalogue view. This is enforced in `toPublicCatalogueDTO`, not only in the page.
