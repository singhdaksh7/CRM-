...
## Maps & Localities
- Provider-independent: `src/integrations/maps/maps-service.ts` dispatches to whichever of `GOOGLE`/`DISABLED` is active — no call site hardcodes Google. `DISABLED` (the current production setting) means every provider method throws a clear config error; callers render a "not configured" state, and no external Maps request is ever attempted.
- **Public catalogue location privacy**: exact property coordinates are never exposed to the public catalogue by default. `Property.publicLocationMode` (`EXACT`/`APPROXIMATE`/`LOCALITY_ONLY`/`HIDDEN`, default `LOCALITY_ONLY`) gates what `catalogue-dto.ts` reveals; `EXACT` only returns real coordinates when the catalogue entry's own address-visibility flag is also enabled — otherwise coordinates are fuzzed to ~1.1km precision (rounded to 2 decimal places) or omitted entirely. This is enforced server-side in the DTO builder, not just hidden in the UI.
- **Client-supplied coordinates are never trusted directly**: manual coordinate entry (`setManualPropertyLocation`) validates via `isValidCoordinates` (rejects NaN/Infinity, out-of-range lat/lng, and the `0,0` "null island" placeholder) before persisting, and is restricted to Admin/Data Manager, audited (`property_coordinates_changed`).
- **Server API key never reaches the browser**: `GOOGLE_MAPS_SERVER_API_KEY` is read only in `src/integrations/maps/google-maps-provider.ts` (server-side); the diagnostics endpoint and Settings UI report only a `configured`/`missing` boolean, never the key value. The separate `NEXT_PUBLIC_GOOGLE_MAPS_BROWSER_API_KEY` is the only Maps credential intentionally shipped to the client, and must be HTTP-referrer restricted in Google Cloud Console (documented in GOOGLE_MAPS_SETUP.md) so a leaked/copied key can't be used from another domain.
- **Cost controls double as abuse controls**: every Maps operation (autocomplete, geocode, reverse geocode, directions, distance matrix, route suggestion, re-verify, health check) is rate-limited per-user via `checkMapsQuota`/`checkRateLimit`, plus a combined per-organization daily cap — protecting against both runaway API billing and a compromised account hammering a paid third-party API. Geocode/directions/distance-matrix results are cached in Redis to further reduce real request volume.
- **Visit conflict overrides are Admin/Data-Manager only and audited** (`visit_conflict_overridden`, with the free-text reason stored) — a Field Executive cannot bypass a detected scheduling conflict, even on their own visit.
