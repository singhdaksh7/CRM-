...
- **Location privacy**: the public catalogue DTO never returns a street address or GPS coordinates. `publicLocationMode` still records what staff intended internally; it does not put a pin on the client-facing catalogue. Exact coordinates remain on the authenticated executive view.
